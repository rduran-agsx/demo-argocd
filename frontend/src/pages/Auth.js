...
import { Box, Button, VStack, Text, Flex, useColorMode, useMediaQuery } from '@chakra-ui/react';
...
const Auth = () => {
  ...
  useEffect(() => {
    if (isAuthenticated && !loading) {
      ...
      if (redirect === '/auth' || redirect === `${window.location.origin}/auth`) {
        redirect = '/';
      }
  ...
  useEffect(() => {
    ...
    if (error) {
      customToast({
        ...
        description: errorMessage || 'There was a problem signing in. Please try again.',
        status: 'error',
        duration: 5000,
        isClosable: true,
      });
  ...
  return (
    ...
    >
      ...
      <Box
        ...
        border="1px solid"
        borderColor={
          colorMode === 'light' ? 'rgba(255,255,255,0.3)' : 'rgba(255,255,255,0.1)'
        }
        boxShadow={
          colorMode === 'light'
